...
!function ($) {
    ...
    CalendarApp.prototype.onDrop = function (eventObj, date) {
        ...
        /* on select */
        CalendarApp.prototype.onSelect = function (start, end, allDay, employees) {
            ...
            form.find(".row")
                .append("<div class='col-md-6'><div class='form-group'><label class='control-label'>Employee</label><select class='form-control' name='title'/></div></div>")
            ...
            $this.$modal.find('form').on('submit', function () {
                ...
                if (title !== null && title.length != 0) {
                    $this.$calendarObj.fullCalendar('renderEvent', {
                        title: form.find("select[name='title'] option:selected").text().split("(").shift(),//employee name
                        start: start,
                        end: end,
                        allDay: false,
    ...
    CalendarApp.prototype.init = function () {
        ...
        $("#btnPostShifts").on("click", function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    swal("Great!", "Shifts were posted", "success").then(function () {
                        window.location.href = "myEmployeesShifts.html";
                    });
        ...
        $("#btnGenerateShifts").on("click", function () {
            myEvents = [];
            var events = $this.$calendarObj.fullCalendar('clientEvents');
            ...
            $.ajax({
                type: "GET",
                url: "api/shifts/optimized/?email=" + Info.Email,
                data: "",
                cache: false,
                contentType: "application/json",
                dataType: "json",
                ...
                success: function (shifts) {
                    ...
                    for (var i = 0; i < shifts.length; i++) {
                        ...
                        var emp = myEmployees.filter(emp => emp.Email == shifts[i].Email);
                        var sJson = {
                    ...
                    $this.$calendar.fullCalendar('addEventSource', myEvents);
                },
                error: function (err) {
                    swal("Something went wrong", err.responseJSON.ExceptionMessage, "error");
                }
        ...
        $.ajax({
            type: "GET",
            url: "api/employees/?managerEmail=" + Info.Email,
            data: "",
            cache: false,
            contentType: "application/json",
            dataType: "json",
            ...
            $.ajax({
                type: "GET",
                url: "api/employees/registered",
                data: "",
                cache: false,
                contentType: "application/json",
                dataType: "json",
                async: true,
                success: function (regEmps) {
                    myEmployees = myEmployees.filter(emp => regEmps.some(reg => reg.Email == emp.Email));
                },
                error: function (err) {
                    swal("Something went wrong", err.responseJSON.ExceptionMessage, "error");
                }
                ...
                $.ajax({
                    ...
                    $.ajax({
                        type: "GET",
                        url: "api/constraints/approval/?email=" + Info.Email,
                        data: "",
                        cache: false,
                        contentType: "application/json",
                        dataType: "json",
                        ...
                        $this.$calendarObj = $this.$calendar.fullCalendar({
                            ...
                            select: function (start, end, allDay, employees) { $this.onSelect(start, end, allDay, myEmployees); },
